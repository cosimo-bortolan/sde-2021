...
class OrdineDAO extends DAO {
  ...
  private function printOrdine($ordine) {
    ...
    $cassa = $cassaDAO->get($ordine->cassa);
    if ($cassa->stampante != null) {
      print_log("Stampante " . $cassa->stampante . " stampa ricevuta");
    }
    $stampanti = $stampanteDAO->getAll();
    $ordine->pietanze = $this->groupPietanze($ordine->pietanze);
    foreach ($stampanti as $stampante) {
      foreach ($stampante->scontrini as $scontrino) {
        $scontrino = $scontrinoDAO->get($scontrino->id);
        if (!empty(array_intersect($scontrino->categorie, $categorie))) {
          stampa($stampante, $scontrino, $ordine);

          //                    $url = 'http://192.168.1.137:8890/printer/192.168.1.1';
          //                    $data = json_encode(array('scontrino' => $scontrino, 'ordine' => $ordine));
          //                    print_log($data);
          //
          //                    $options = array(
          //                        'http' => array(
